...
// Organizando os atributos do objeto 'game' do Phaser
    ...
    var player, keys, fruits;

    // Carrega para a memoria do dispositivo
    function preload(){
        this.load.image('floresta', 'img/foresta.png');
        this.load.spritesheet('boy','img/PBoy.png',{frameWidth:48, frameHeight:54});
    ...
    function create(){
        ...
        fruits = this.add.group(); //cria o grupo de frutos
        fruits.enableBody = true; //Ativa um corpo para as frutas
        fruits.physicsBodyType = Phaser.Physics.ARCADE; //Defini a fisica das frutas para Arcade

        var c = fruits.create(Phaser.Math.Between(0,400), 50, 'fruit', Phaser.Math.Between(0,7));
